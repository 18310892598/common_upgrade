...
//    private Subscription subscription;
    ...
    public class MyBinder extends Binder {
        ...
        public void startDownload(String url, String filePath, int type) {
            ...
            if (subscription == null) {
                  NetWork.getApi(new ProgressListener() {
                    @Override
                    public void onProgress(float section, float total, boolean done) {
                        ...
                        .subscribe(new Observer<Boolean>() {// Android 主线程，由 observeOn() 指定
    ...
    private static final String CHANNEL_ID_KEY = "cn.flyexp.backgroundupdate";
    ...
    private void showNotification(int progress, boolean isSuccess) {
        ...
        if (mNotifyBuilder == null) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                notificationManager.createNotificationChannel(new NotificationChannel(getPackageName(), "欧了通知", NotificationManager.IMPORTANCE_LOW));
            }

            mNotifyBuilder = (NotificationCompat.Builder) new NotificationCompat.Builder(this,getPackageName())
                    .setSmallIcon(R.drawable.logo)  //系统状态栏显示的小图标
                    .setContentTitle("正在下载...")            //通知栏标题
                    .setAutoCancel(false)       //不可点击通知栏的删除按钮删除
                    .setWhen(System.currentTimeMillis()) //通知的时间
                    .addAction(R.drawable.ic_close, "取消", cancelIntent);    //添加一个取消按钮
        }
        if (isSuccess) {
